...
- **Tracing.** OpenTelemetry traces go to `SLUICE_OTLP_ENDPOINT` (Jaeger in the local stack, at http://localhost:16686). The trace context of the submitting request is stored with the job, so a job's `worker.execute` span belongs to the same trace as its `POST /v1/jobs`, even when the job runs hours later. An incoming W3C `traceparent` header is honoured, so the trace can start in your own service.
